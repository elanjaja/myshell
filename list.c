 ...
 * Return: size of list
 */
list_t *add_node_at_end(list_t **head, const char *str, int index)
{
...
/**
 * print_list_string - prints only the string value of a list_t linked list
 * @n: pointer to first node
 *
 * Return: size of list
 */
size_t print_list_string(const list_t *n)
{
	size_t a = 0;

	while (n)
	{
		print_string(n->str ? n->str : "(nil)");
		print_string("\n");
		n = n->next;
		a++;
	}
...
/**
 * free_list_node - frees all nodes of a list
